---
title: "fec12"
author: "Marium Tapal and Ben Baumer"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{fec12}
...
The `fec12` package provides tamed relational datasets of the Federal Election Commission (FEC)'s 2011-2012 election cycle. Some datasets are included in full, while a sample of the others is available with the option of retrieving the entire datasets through the functions built in the package. Details are given below.
...
Anyone interested in US politics and elections who wants to use actual data to think critically and make inferences. We made this package particularly with students and instructors in mind as there is demand for relational data in teaching. `fec12` is a one-stop shop for acquiring data of this kind.
...
all_expenditures <- fec12::read_all_expenditures()

# The first 30 entries in this dataset can be accessed by:
expenditures_30 <- fec12::read_all_expenditures(n_max = 30)
```
...
Using the `results_president` dataset, we can also see what actually happened in the 2012 elections.
...
Compare these results to [those reported by the Wikipedia](https://en.wikipedia.org/wiki/2012_United_States_presidential_election).
...
all_contributions <- fec12::read_all_contributions()
...
ggplot(results_by_cand, aes(x = contribution, y = sum_votes)) +
  ...
  labs(
    title = "Contributions vs. Votes in 2012",
